...
const seoPages = {
  '/': ['Apna Bazar | Fashion for Women, Men & Kids', 'Shop everyday fashion for women, men and kids at Apna Bazar. Discover clothing, new arrivals and sale styles with secure checkout.'],
  '/shop': ['Shop Clothing Online | Apna Bazar', 'Explore clothing for women, men and kids. Shop new arrivals, trusted brands and sale styles at Apna Bazar.'],
  '/about': ['About Apna Bazar | Everyday Fashion', 'Meet Apna Bazar and discover our approach to affordable, considered fashion for everyday life.'],
  '/contactus': ['Contact Apna Bazar | Customer Support', 'Contact the Apna Bazar support team for product, delivery and order assistance.'],
  '/testimonials': ['Customer Stories | Apna Bazar', 'Read customer experiences and stories from the Apna Bazar community.'],
};
function setMeta(selector, attributes) { let element = document.head.querySelector(selector); if (!element) { element = document.createElement('meta'); document.head.appendChild(element); } Object.entries(attributes).forEach(([key, value]) => element.setAttribute(key, value)); }
function Seo() {
  const { pathname } = useLocation();
  useEffect(() => {
    const privatePage = /^\/(login|signup|cart|checkout|order-confirmation|profile|wishlist|admin)/.test(pathname);
    const [title, description] = seoPages[pathname] || (pathname.startsWith('/product/') ? ['Product Details | Apna Bazar', 'View product details, available sizes, colours and stock at Apna Bazar.'] : ['Apna Bazar | Everyday Fashion Online', 'Discover clothing for women, men and kids at Apna Bazar.']);
    const canonicalUrl = `https://ecoproject-rho.vercel.app${pathname === '/' ? '' : pathname}`;
    document.title = title;
    setMeta('meta[name="description"]', { name: 'description', content: description });
    setMeta('meta[name="robots"]', { name: 'robots', content: privatePage ? 'noindex, nofollow' : 'index, follow, max-image-preview:large' });
    setMeta('meta[property="og:title"]', { property: 'og:title', content: title });
    setMeta('meta[property="og:description"]', { property: 'og:description', content: description });
    setMeta('meta[property="og:url"]', { property: 'og:url', content: canonicalUrl });
    setMeta('meta[property="og:type"]', { property: 'og:type', content: pathname.startsWith('/product/') ? 'product' : 'website' });
