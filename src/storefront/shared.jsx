...
export function Provider({ children }) {
  ...
  useEffect(() => { const expired = () => { setUser(null); setCart({ items: [], subtotal: 0, total: 0, shipping: 0 }); setWishlist([]); notify('Please sign in to continue.'); const from = window.location.pathname + window.location.search; if (!window.location.pathname.startsWith('/login')) navigate('/login', { state: { from }, replace: true }); }; window.addEventListener('apna:unauthorized', expired); return () => window.removeEventListener('apna:unauthorized', expired); }, [navigate, notify]);
...
export function ProductCard({ product: p, onQuickView }) {
